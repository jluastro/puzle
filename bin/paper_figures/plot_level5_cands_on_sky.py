...
def plot_cands_on_sky():
    cands = CandidateLevel4.query.\
        ...
        with_entities(CandidateLevel4.ra, CandidateLevel4.dec, CandidateLevel4.category).\
        all()
    ...
    coords_cands = SkyCoord(ra_arr, dec_arr, frame='icrs', unit='degree')
    glon_arr = coords_cands.galactic.l.value
    lb_cond = glon_arr >= 180
    glon_arr[lb_cond] -= 360
    glat_arr = coords_cands.galactic.b.value

    ra_ztf = np.linspace(0, 360, 10000)
    dec_ztf = np.ones(len(ra_ztf)) * -20
    ...
    ax.scatter(glon_arr[clear_cond], glat_arr[clear_cond], c='r', s=5)
    ax.set_xlabel('glon', fontsize=16)
    ax.set_ylabel('glat', fontsize=16)
    ax.scatter(glon_ztf, glat_ztf, color='k', s=1, alpha=.2)
    ax.set_xlim(-180, 180)
    ax.set_ylim(-90, 90)
    fig.tight_layout()
...
def plot_cands_tE_piE_overlapping_popsycle():
    ...
    ax.scatter(tE_cands[cond_piE_err], piE_cands[cond_piE_err], color='g')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlabel(r'$t_E$', fontsize=16)
